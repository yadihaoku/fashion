...
public class LnImpl implements LnInterface {
    ...
    /**
     * android Log 有长度限制 (4076)，过长的话，就截断输出。
     * 为了保持日志的完整性，在打印日志时。如果过长，则分次打印。
     * 所以我们限制为1024x3 个字符，因为tag + time + thread 的长度是未知的，所以限制一个较小的长度。
     */
    private static final int S_LOG_MAX_PAYLOAD = 1024 * 3;
     ...
     * @param msg
     */
    public static int cutLog(int priority, String tag, String msg) {
        int len = msg.length();
        if (len > S_LOG_MAX_PAYLOAD) {
            int cutCount = len / S_LOG_MAX_PAYLOAD;
            if (len % S_LOG_MAX_PAYLOAD != 0)
                cutCount++;

            for (int i = 0; i < cutCount; i++) {
                int start = i * S_LOG_MAX_PAYLOAD;
                if (i == cutCount - 1) {
                    return Log.println(priority, null, msg.substring(start));
                } else {
                    return Log.println(priority, i == 0 ? tag : null, msg.substring(start, start + S_LOG_MAX_PAYLOAD));
                }
            }
        } else {
    ...
    public int v(Object s1, Object... args) {
        ...
        final String s = TextUtils.toString(s1);
    ...
    public int println(int priority, String msg) {
        String mixedMsg = processMessage(msg);
        return cutLog(priority, getTag(), mixedMsg);
    }
    ...
    protected String getTag() {
        ...
        if (getLoggingLevel() <= Log.DEBUG) {
            ...
            if (traceElements.length > skipDepth) {
                final StackTraceElement trace = traceElements[skipDepth];
